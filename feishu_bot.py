...
class FeishuBot:
    ...
    def __init__(self, app_id: str, app_secret: str, clawcode: ClawCodeExecutor):
        ...
        dedup_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data")
        self._dedupe = dedup.PersistentDedupe(
            dedup_dir=dedup_dir,
    ...
    def handle_message(self, event, data):
        ...
        try:
            ...
            if msg_type == "text" and user_text:
                ...
                # 跳过机器人消息（防止被其他 Bot 的 cron 简报触发重复响应）
                sender_type = getattr(event.sender, 'sender_type', None) or getattr(event.sender, 'type', None)
                if sender_type == 'bot':
                    logger.info(f"跳过机器人消息 msg_id={msg_id} sender_type={sender_type}")
                    return
                logger.info(f"收到消息 msg_id={msg_id} from {sender_id}: {user_text[:50]}")

                # OpenClaw 风格去重：内存+磁盘两层检查
                if self._dedupe.check_and_record(msg_id):
    ...
    def _process_single(self, task):
        ...
        try:
            ...
            if processing_msg_id:
                try:
                    updated = self.update_card_message(processing_msg_id, result_card)
                    if not updated:
                        self.send_interactive_card(chat_id, result_card)
                except Exception as e:
                    logger.warning(f"更新卡片失败，降级为新卡片: {e}")
                    self.send_interactive_card(chat_id, result_card)
            else:
                self.send_interactive_card(chat_id, result_card)
